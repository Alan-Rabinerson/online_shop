...
if ($result) {
    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/student024/Shop/backend/logs/supplier_response.log', date('Y-m-d H:i:s') . " - Supplier ID: $supplier_id - Response: $result\n", FILE_APPEND);
} else {
    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/student024/Shop/backend/logs/supplier_response.log', date('Y-m-d H:i:s') . " - Supplier ID: $supplier_id - No response received.\n", FILE_APPEND);
}
